...
export const articleSchema = (input: ArticleSchemaInput) => ({ '@context': 'https://schema.org', '@type': 'Article', headline: input.title, description: input.description, url: absoluteUrl(input.path), datePublished: input.publishedAt.toISOString(), dateModified: (input.updatedAt ?? input.publishedAt).toISOString(), inLanguage: 'zh-CN', author: { '@type': 'Organization', name: '找机场编辑团队', url: absoluteUrl('/author/') }, publisher: { '@type': 'Organization', name: site.name, url: site.url }, ...(input.image ? { image: absoluteUrl(input.image) } : {}) });
