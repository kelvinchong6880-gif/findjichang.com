...
为什么浏览器能科学上网，系统自带应用却像断了网一样？这背后的罪魁祸首，其实是 Windows 系统底层的安全机制。本文将为您深度剖析原因，并提供最较好的解决方案。
...
## 三、 完整杀招：TUN 虚拟网卡模式全景指南


如果您觉得通过工具逐一勾选 UWP 应用过于繁琐，或者您同时还是外服网游玩家、重度命令行（CMD/Git）使用者，那么开启代理客户端的 **TUN（虚拟网卡）模式** 是一劳永逸的完整方案。
...
- **较好绕过限制**：因为流量不再发往本地的回环端口，而是直接走虚拟网卡物理级通道，Windows 的网络隔离机制被彻底绕过。
