...
originalValue: "围绕“节点、订阅、配置文件和规则集有什么区别”提供独立判断框架、反例和操作清单"
...
先说容易被忽略的一点：节点描述一次连接参数；订阅是批量配置入口；配置文件组合节点、DNS和路由；规则集只负责判断流量去向。 因此，“节点、订阅、配置文件和规则集有什么区别”不是记住术语就结束，而是要知道每项证据能证明到哪里。
...
| 要核对的动作 | 合格记录 | 仍然不能证明 |
| --- | --- | --- |
| 确认错误发生层级 | 写明时间、环境和原始结果 | 永久稳定或全国一致 |
| 备份本地覆盖项 | 固定其他变量后再比较 | 差异一定由单一原因造成 |
| 检查内核支持 | 至少换一个时段复现 | 平台策略以后不会变化 |
| 分别测试节点和规则 | 先设定可承受的风险边界 | 服务商能够兑现所有宣传 |
...
3. **每次只改一项。** 按“确认错误发生层级 → 备份本地覆盖项 → 检查内核支持 → 分别测试节点和规则”推进，失败时可以退回上一步。
4. **跨时段复核。** 对性能和平台识别问题，至少在另一个时段重复。
5. **写清结论强度。** 使用“已确认、较可能、仍未知”，不要把一次结果写成永久保证。

## 停止排错的条件

- **把规则集当节点来源。** 这会跳过必要条件，并把线索升级成结论。
- **订阅失败就反复重装。** 页面名称、群人数或检测标签并不等于独立证明。
- **直接编辑托管配置后期待永久保留。** 短期便利不应换来不可逆的账号、隐私或长期预付风险。
...
- [sing-box 配置文档](https://sing-box.sagernet.org/configuration/)：核对本文涉及的基础定义或平台规则。
