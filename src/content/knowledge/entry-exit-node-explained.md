...
先说容易被忽略的一点：入口是用户先连接的位置，落地或出口是访问目标网站时显示的网络位置，中间还可能经过转发链路。 因此，“入口、落地、出口节点分别是什么意思”不是记住术语就结束，而是要知道每项证据能证明到哪里。
...
| 要核对的动作 | 合格记录 | 仍然不能证明 |
| --- | --- | --- |
| 查看连接日志 | 写明时间、环境和原始结果 | 永久稳定或全国一致 |
| 检查出口 IP | 固定其他变量后再比较 | 差异一定由单一原因造成 |
| 用路由跟踪辅助判断 | 至少换一个时段复现 | 平台策略以后不会变化 |
| 向服务商核对节点说明 | 先设定可承受的风险边界 | 服务商能够兑现所有宣传 |
...
3. **每次只改一项。** 按“查看连接日志 → 检查出口 IP → 用路由跟踪辅助判断 → 向服务商核对节点说明”推进，失败时可以退回上一步。
4. **跨时段复核。** 对性能和平台识别问题，至少在另一个时段重复。
5. **写清结论强度。** 使用“已确认、较可能、仍未知”，不要把一次结果写成永久保证。

## 停止排错的条件

- **把香港节点理解为全程都在香港。** 这会跳过必要条件，并把线索升级成结论。
- **只测出口 IP 不看入口质量。** 页面名称、群人数或检测标签并不等于独立证明。
- **认为节点名称就是可验证拓扑。** 短期便利不应换来不可逆的账号、隐私或长期预付风险。
