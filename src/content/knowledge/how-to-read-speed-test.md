...
description: "建立能读懂测速结果、发现测试偏差并复现结论的方法。"
...
originalValue: "围绕“怎么看机场测速：延迟、抖动、丢包和带宽”提供独立判断框架、反例和操作清单"
...
先说容易被忽略的一点：一张测速图只能说明特定时间和条件下的样本；延迟、抖动、丢包和吞吐量必须结合应用场景、测试方法与本地基线一起解释。 因此，“怎么看机场测速：延迟、抖动、丢包和带宽”不是记住术语就结束，而是要知道每项证据能证明到哪里。
...
| 要核对的动作 | 合格记录 | 仍然不能证明 |
| --- | --- | --- |
| 先测本地直连基线 | 写明时间、环境和原始结果 | 永久稳定或全国一致 |
| 固定节点与测速服务器 | 固定其他变量后再比较 | 差异一定由单一原因造成 |
| 连续测试多个时段 | 至少换一个时段复现 | 平台策略以后不会变化 |
| 用真实应用交叉验证 | 先设定可承受的风险边界 | 服务商能够兑现所有宣传 |
...
3. **每次只改一项。** 按“先测本地直连基线 → 固定节点与测速服务器 → 连续测试多个时段 → 用真实应用交叉验证”推进，失败时可以退回上一步。
4. **跨时段复核。** 对性能和平台识别问题，至少在另一个时段重复。
5. **写清结论强度。** 使用“已确认、较可能、仍未知”，不要把一次结果写成永久保证。

## 停止排错的条件

- **只看绿色延迟数字。** 这会跳过必要条件，并把线索升级成结论。
- **把他人的测速图当成自己的预期。** 页面名称、群人数或检测标签并不等于独立证明。
- **忽略测试时间、节点和服务器。** 短期便利不应换来不可逆的账号、隐私或长期预付风险。
